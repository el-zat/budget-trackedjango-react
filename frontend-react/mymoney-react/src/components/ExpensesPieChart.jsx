...
const ExpensesPieChart = () => {
    ...
    const pieSegments = useMemo(() => {
        ...
        return expensesArray.map((expense, index) => {
            const angle = (parseFloat(expense.percent) / 100) * 360
                ...
                <div className="diagram-content">
                    ...
                    <div className="pie-legend">
                        {expensesArray.map((expense, index) => (
                            <div key={expense.id} className="legend-item">
                                <span 
                                    className="legend-color" 
                                    style={{ backgroundColor: getExpenseColor(index) }}
                                ></span>
                                <span className="legend-name">{expense.name}</span>
                                <span className="legend-sum">€ {expense.sum.toFixed(2)}</span>
                                <span className="legend-percent">{expense.percent}%</span>
                            </div>
                        ))}
                    </div>
